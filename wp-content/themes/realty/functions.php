...
function tt_realty_scripts() {
	...
	*/
	...
	if ( is_page_template( 'single-property.php' ) || is_page_template( 'template-property-slideshow.php' ) || is_page_template( 'template-home-slideshow.php' ) ) {
	//wp_enqueue_script( 'tt-lazy-load', get_template_directory_uri() . '/assets/js/lazyloader.js', array( 'jquery' ), null, true ); 
	}
	...
	// Map Style
	//$tt_map_style = array( 'map_style' => $realty_theme_option['map-style'] );
	...
	if ( $realty_theme_option['datepicker-language'] && $realty_theme_option['datepicker-language'] != 'en' ) {
		wp_enqueue_script( 'datepicker-'. $realty_theme_option['datepicker-language'], get_template_directory_uri() . '/assets/js/bootstrap-datepicker/locales/bootstrap-datepicker.' . $realty_theme_option['datepicker-language'] . '.js', array('tt-theme-main' ), null, true ); 
	}
	...
	if ( ! $realty_theme_option['property-favorites-disabled'] && $realty_theme_option['property-favorites-temporary'] ) {	
		wp_enqueue_script( 'ajax-favorites-temporary', get_template_directory_uri() . '/assets/js/ajax-favorites-temporary.js', array( 'tt-theme-main' ), null, false );
	}
...
/* End Thumbnail size in wordpress 4.2.2. */
/* Sidebars
============================== */
...
function tt_add_lost_password_link() {
	return '<a href="' . wp_lostpassword_url(get_permalink()) . '">'.__("Lost Password?").'</a>';
}
...
function tt_custom_styles() {
	...
	$home_slideshow_custom_text_align = $realty_theme_option['home-slides-text-center'];
	$home_slideshow_custom_overlay_opacity = $realty_theme_option['home-slides-overlay-opacity'];
	...
	if ( $home_slideshow_custom_text_align ) {
		echo "#home-slideshow.slideshow-custom { text-align: $home_slideshow_custom_text_align; }\n";
	}
	if ( $realty_theme_option['home-slides-overlay-opacity'] ) {
		echo "#home-slideshow .wrapper:after { opacity: $home_slideshow_custom_overlay_opacity; }";
	}
...
if ( $realty_theme_option['home-slideshow-speed'] ) { 
?>
...
if ( isset( $datepicker_language ) ) { ?>
		...
		autoclose: true,
		isRTL: <?php if($realty_theme_option['enable-rtl-support']){ echo "true"; } else { echo "false"; } ?>,
		format: "yyyymmdd",
		});
<?php } ?>
// Price Range
