...
$template_page_property_user_loign_array = get_pages( array (
	'meta_key' => '_wp_page_template',
	'meta_value' => 'template-user-login.php'
	)
);
if ( $template_page_property_user_loign_array  ) {
	
	$user_login_page = $template_page_property_user_loign_array[0]->ID;
}
else {
 $user_login_page='#login-modal';
}
?>
<div id="login-bar-header" class="primary-tooltips">
	...
	<?php } ?>
	<a href="<?php if($realty_theme_option['header-login-modal-layout']=='login-page'){ echo get_permalink( $user_login_page ); }else{ echo '#login-modal'; } ?>" data-toggle="modal"><?php if( get_option('users_can_register') ) { _e( 'Login/Register', 'tt' ); } else { _e( 'Login', 'tt' ); } ?></a>
	<?php }	
	// Logged-In User
	else {
