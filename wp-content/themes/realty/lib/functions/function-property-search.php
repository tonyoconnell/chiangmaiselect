...
function tt_property_search_args($search_results_args) {
	...
	foreach ( $_GET as $search_key => $search_value ) { 
		...
		if ( ( ! empty( $search_value ) || $search_key == "price_range_min" ) && $search_key != "order-by" && $search_key != "pageid" ) {
			...
			if (isset($search_fields[$i]) && in_array( $search_fields[$i], $default_search_fields_array ) ) {
				...
				switch ( $search_fields[$i] ) {
					...
					if ( ( $_GET['price_range_min'] == $realty_theme_option['property-search-price-range-min'] || $_GET['price_range_min'] == 0 ) && $_GET['price_range_max'] = $realty_theme_option['property-search-price-range-max'] ) {
						break;
					} else {
			...
			else if ( tt_acf_active() && isset($search_fields[$i]) && in_array( $search_fields[$i], tt_acf_fields_name( tt_acf_group_id_property() ) ) ) {
